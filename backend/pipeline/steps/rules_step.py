...
class RulesStep(AnalysisStep):
    ...
    def execute(self, context: Any) -> Any:
        ...
        context.data.rule_score = rule_result["risk_score"]
        context.data.rule_label = rule_result["risk_label"]
        context.data.reasons = rule_result["reasons"]
        context.data.suggested_action = get_suggested_action(str(rule_result["risk_label"]))
        return self._ok({
            "rule_score": rule_result["risk_score"],
            "rule_label": rule_result["risk_label"],
            "reasons": rule_result["reasons"],
            "suggested_action": context.data.suggested_action,
        })
